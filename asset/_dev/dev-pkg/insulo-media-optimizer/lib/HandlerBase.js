...
class HandlerBase {
    ...
    clean() {
        if(fs.existsSync(this.dist)) {
            if(fs.unlinkSync(this.dist)) {
                log.raw('MediaHandler: removed dist: ' + colors.underline(this.name));
            } else {
    ...
    run_internal(handle) {
        return new Promise((resolve => {
            let start = log.start('MediaHandler: ' + colors.underline(this.name));
            ...
            if(false === fs.existsSync(path.dirname(this.dist))) {
                if(fs.mkdirSync(path.dirname(this.dist), {recursive: true})) {
                    log.raw('MediaHandler: could not create dist dir: ' + colors.underline(path.dirname(this.dist)));
                }
            }
            ...
            handle(err => {
                ...
                log.end('MediaHandler: ' + colors.underline(this.name), start, new Date(), saved_msg);
                resolve({
                    'file': this.name,
                    'before': size_before,
                    'after': size_after,
